...
def generate_rss():
    ...
    for subject in subjects:
        page = 1
        while True:
            should_break = False  # 标志变量，用于控制是否跳出外层循环
            url = base_url_template.format(subject, page)
            retries = 0
            max_retries = 3  # 每页最大重试次数
            while retries < max_retries and not should_break:
                try:
                    ...
                    if retries < max_retries:
                        print(f"学科 {subject}，页面 {page} 抓取失败：{e}（第 {retries}/{max_retries} 次重试）")
                        time.sleep(10 * retries)  # 指数退避，重试前等待更长时间
                        continue
                    else:
                        print(f"学科 {subject}，页面 {page} 抓取失败（已达最大重试次数 {max_retries}）：{e}")
                        break  # 达到最大重试次数，跳出当前页循环
            if should_break:
                break  # 跳出外层 while True 循环，进入下一个学科

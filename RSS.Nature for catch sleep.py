...
def generate_rss():
    ...
    # 限制抓取前三页（page=1, page=2, page=3）
    for page in range(1, 4):  # range(1, 4) 生成 1、2、3
        ...
        time.sleep(2)
    # 按日期降序排序（新文章在前）
    all_articles.sort(key=lambda x: datetime.strptime(x.select_one('time')['datetime'], '%Y-%m-%d'), reverse=True)

    fg = FeedGenerator()
    fg.title('Nature Vibration Metamaterials RSS')
    fg.link(href=base_url.format(1), rel='alternate')  # 使用第一页 URL 作为主链接
    fg.description('Latest articles from Nature with both "vibration/vibrations" and "metamaterial/metamaterials" (first 3 pages)')
